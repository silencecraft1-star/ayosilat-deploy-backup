                ...
                <tbody class="text-start">
                    <tr>
                        ...
                        @if ($score > $score_check)
                            <td id="{{ $status }}" class="w-10 fw-bold text-danger align-middle text-center">-{{$score}}
                            </td>
                        @else
                            <td id="{{ $status }}" class="w-10 fw-bold text-primary align-middle text-center">{{$score}}
                            </td>
                        @endif
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        @endphp
                        @if ($score > $score_check)
                            <td id="{{ $status }}" id="{{ $status }}"
                                class="w-10 fw-bold text-danger align-middle text-center">-{{$score}}</td>
                        @else
                            <td id="{{ $status }}" class="w-10 fw-bold text-primary align-middle text-center">{{$score}}
                            </td>
                        @endif
                    ...
                    <tr>
                        ...
                        @php
                            $score = score::where('keterangan', $status)->where('id_perserta', $id_perserta)
                                ->where('arena', $arena)
                                ->where('partai', $partai)->sum('score');
                            $score = number_format($score, 2);
                            $score_check = number_format(0, 2);
                        @endphp
                        @if ($score > $score_check)
                    ...
                    <tr>
                        ...
                        @php
                            $score = score::where('status', "seni_minus")->where('id_perserta', $id_perserta)
                                ->where('arena', $arena)
                                ->where('partai', $partai)
                                ->sum('score');
                            $score = number_format($score, 2);
        ...
        <div class="row">
            <div class="col-12 d-flex justify-content-end gap-2">
                ...
                <form action="{{route('rekap.seni.data')}}" method="post">
                    ...
                    <input type="text" hidden value="tunggal" name="kategori">
                    <button class="btn btn-lg btn-success">Simpan Pertandingan</button>
                </form>
